...
<head>
	...
	<script type="text/javascript">
		...
		$(function () {
			$("#toMyCenter").click(checkRealName);
		});
		//校验用户的真实姓名
    ...
    <div class="registerBody">
		...
		<c:choose>
			...
			<c:otherwise>
				...
				<div class="bn_register"><a href="${pageContext.request.contextPath}/loan/myCenter" class="btn-1" id="toMyCenter">进入我的小金库</a></div>
				</div>
			</c:otherwise>
		</c:choose>
...
<div id="productArea">
	...
	<div class="index-exper">
		<c:forEach var="loanInfoX" items="${loanInfoListX}">
			 <div class="inner">
				  <div class="index-exper-top">
					   <span class="title"><font style="font-size:28px;">${loanInfoX.productName}</font></span>
					   <p class="text">${loanInfoX.bidMinLimit}元起投<em>丨
					   </em>投资最高限额${loanInfoX.bidMaxLimit}元<em>丨</em>当日计息</p>
				  </div>
				  <div class="index-exper-bottom">
					   <ul class="list cf">
						   <li class="box1">
							   <p class="text1">${loanInfoX.rate}<span>%</span></p>
							   <p class="text2">历史年化收益率</p>
						   </li>
						   <li class="box2">
							   <p class="text1">${loanInfoX.cycle}<span>天</span></p>
							   <p class="text2">投资周期</p>
						   </li>
						   <li class="box3">
							   <p class="text1">${loanInfoX.leftProductMoney}<span>元</span></p>
							   <p class="text2">剩余可投金额</p>
						   </li>
						   <li class="box4">
							   <a href="loan/loanInfoDetail?productId=${loanInfoX.id}" class="go-invest">立即投资</a>
						   </li>
					   </ul>
				  </div>
			 </div>
	...
	<div class="mainBox pro-body clearfix">
		<c:forEach var="loanInfoU" items="${loanInfoListU}">
			<!-- 1:已满标时class样式追加 pro-full-->
			<div class="pro-box <c:if test='${loanInfoU.leftProductMoney eq  0}'>pro-full</c:if>">
				<div class="pro-top">
					<h3>${loanInfoU.productName}</h3>
					<p>短期小额信用借款集合投资计划</p>
				</div>
				<div class="pro-main">
					<div class="pro-rate">
						<div class="rate">
							${loanInfoU.rate}<span>%</span>
						</div>
						<h3>历史年化收益率</h3>
					</div>
					<div class="pro-data clearfix">
						<div class="pro-cycle">
							<h3>投资周期</h3>
							<div class="data"><span>${loanInfoU.cycle}</span>个月</div>
						</div>
						<div class="pro-money">
							<h3>剩余可投金额</h3>
							<div class="data">
								<span>${loanInfoU.leftProductMoney}</span>元
							</div>
						</div>
					</div>
					<div class="pro-intr"><p>优选计划项目，投资回报周期1个月，起点低，适合短期资金周转、对流动性要求高的投资人。</p></div>
					<div class="pro-btn">
						<a href="loan/loanInfoDetail?productId=${loanInfoU.id}" class="btn-1">立即投资</a>
					</div>
				</div>
			</div>
		</c:forEach>
	...
	<div class="pro-more"><i class="left-i"></i><a href="loan/loan?ptype=1&pageSize=9">查看更多优选类产品</a><i class="right-i"></i></div>
	<!-- 优选计划end -->
	
	<!-- 散标start -->
	<div class="mainBox pro-body disperse-pro clearfix">
	  	<div class="new-pro-icon"><img src="images/new-pro-icon.png"/></div>
		<c:forEach items="${loanInfoListS}" var="loanInfoS" varStatus="index">

			<!-- 1:已满标时追加pro-full-->
			<div class="pro-box <c:if test='${loanInfoS.leftProductMoney eq 0}'>pro-full</c:if>">
				<div class="pro-top">
					<h3>${loanInfoS.productName}</h3>
					<p>动力金融网特选小额理财产品</p>
				</div>
				<div class="pro-main">
					<div class="pro-rate">
						<div class="rate">${loanInfoS.rate}<span>%</span></div>
						<h3>历史年化收益率</h3>
					</div>
					<div class="pro-data clearfix">
						<div class="pro-cycle">
							<h3>投资周期</h3>
							<div class="data"><span>${loanInfoS.cycle}</span>个月</div>
						</div>
						<div class="pro-money">
							<h3>剩余可投金额</h3>
							<div class="data">
								<span>${loanInfoS.leftProductMoney}</span>元
							</div>
						</div>
					</div>
					<div class="pro-btn">
						<a href="loan/loanInfoDetail?productId=${loanInfoS.id}" class="btn-1">立即投资</a>
					</div>
				</div>
			</div>
	...
	<div class="pro-more"><i class="left-i"></i><a href="loan/loan?ptype=2&pageSize=9">查看更多散标类产品</a><i class="right-i"></i></div>
	<!-- 散标end -->
</div>
<!--投资产品end-->
